...
const int ledPinBlue = 9;       // RESUB: Added second LED
...
void setup () {
  ...
  pinMode(ledPinBlue, OUTPUT);            // RESUB: Added second LED
...
void wave() { 
  if (currentLedState != HIGH) {                    // Prevents spam
    ledState(ledPinGreen, HIGH);
    ledState(ledPinBlue, HIGH);                   // RESUB: Added second LED                     
...
void pat() { 
  if (currentLedState != LOW) {                    // Prevents spam
    ledState(ledPinGreen, LOW);
    ledState(ledPinBlue, LOW);                     // RESUB: Added second LED
